...
Votre avis compte pour bâtir un service public de qualité <span>💪</span>
</div>
...
<script>
    document.addEventListener('DOMContentLoaded', () => {
        const formFields = document.querySelectorAll('#registrationForm input:not([type="button"]), #registrationForm select, #registrationForm textarea');
        const sections = document.querySelectorAll('.section');
        ...
        formFields.forEach(field => {
            ...
            if (savedValue) {
                if (field.type === 'radio') {
                    document.querySelector(`input[name="${field.name}"][value="${savedValue}"]`).checked = true;
                    if (savedValue === 'Diaspora') {
                        document.getElementById('diasporaCountries').style.display = 'block';
                        const countrySaved = localStorage.getItem('country');
                        if (countrySaved) {
                            document.getElementById('country').value = countrySaved; // Récupérer le pays sélectionné
                        }
                    } else {
                        document.getElementById('diasporaCountries').style.display = 'none';
                    }
            ...
            field.addEventListener('change', () => {
                if (field.type === 'radio') {
                    localStorage.setItem(field.name, field.value);
                    // Afficher ou cacher le sélecteur de pays
                    if (field.value === 'Diaspora') {
                        document.getElementById('diasporaCountries').style.display = 'block';
                        const countrySaved = localStorage.getItem('country');
                        if (countrySaved) {
                            document.getElementById('country').value = countrySaved; // Récupérer le pays sélectionné
                        ...
                        document.getElementById('diasporaCountries').style.display = 'none';
                        localStorage.setItem('country', ''); // Réinitialiser le pays
                    }
                }
            });
        });

        // Afficher la bonne section au démarrage
        let activeSectionIndex = parseInt(localStorage.getItem('activeSectionIndex')) || 1;
        showSection(activeSectionIndex);

        // Navigation entre les sections
        document.querySelectorAll('#registrationForm button[type="button"]').forEach(button => {
        ...
        // Gestion de l'affichage du sélecteur de pays selon le choix de résidence
        document.querySelectorAll('input[name="location"]').forEach(radio => {
            radio.addEventListener('change', () => {
                const diasporaCountries = document.getElementById('diasporaCountries');
                if (radio.value === 'Diaspora') {
                    diasporaCountries.style.display = 'block';
                    const countrySaved = localStorage.getItem('country');
                    if (countrySaved) {
                        document.getElementById('country').value = countrySaved; // Récupérer le pays sélectionné
                    }
                } else {
                    diasporaCountries.style.display = 'none';
                    localStorage.setItem('country', ''); // Réinitialiser le pays
                }
        ...
        // Effacer les données stockées lorsque le formulaire est soumis
