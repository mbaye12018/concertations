...
<div class="left-section" id="left-section">
    ...
    <a class="sector-button" onclick="chooseSection(1)">
        <i class="fas fa-comments"></i>&nbsp; Santé
    </a>
    <a class="sector-button" onclick="chooseSection(2)">
        <i class="fas fa-cog"></i>&nbsp; Éducation
    </a>
    <a class="sector-button" onclick="chooseSection(3)">
        <i class="fas fa-users"></i>&nbsp; Sécurité
    </a>
    <a class="sector-button" onclick="chooseSection(4)">
        <i class="fas fa-users"></i>&nbsp; Autres
    </a>
</div>

<!-- Contenu des questions -->
<div id="sectorTitle"></div>
<div id="questionsList"></div>

<!-- Zone spécifique pour 'Autres' avec le select -->
<div id="otherSectorSelectContainer" style="display: none;">
    <label for="otherSectorSelect">Choisissez un autre secteur :</label>
    <select id="otherSectorSelect">
...
<!-- Div où les questions seront affichées -->
...
/* Cacher le select et les questions au début */
#otherSectorSelectContainer {
    display: none;
}
...
<script>
    function chooseSection(sectionId) {
        ...
        const otherSectorSelectContainer = document.getElementById('otherSectorSelectContainer');
        const otherSectorSelect = document.getElementById('otherSectorSelect');
        
        // Cacher le select et les questions initialement
        otherSectorSelectContainer.style.display = 'none';
        questionsList.innerHTML = '';

        // En fonction de la section choisie
        switch (sectionId) {
            case 1:
                sectorTitle.innerText = "Questions sur la Santé";
                questionsList.innerHTML = `
                    <div class="question">
                        <label><strong><p>1. Comment évaluez-vous la qualité des soins de santé ?</p></strong></label>
                ...
                <span class="star" data-value="5">★</span>
                    ...
                    <p>2. Quels sont les principaux défis rencontrés dans le système de santé ?</p>
                    <input type="text" name="health_challenges" id="health_challenges" placeholder="Décrivez les défis rencontrés" style="width: 100%; padding: 8px;">
                `;
                break;

            case 2:
                sectorTitle.innerText = "Questions sur l'Éducation";
                questionsList.innerHTML = `
                    <p>1. Comment évaluez-vous la qualité de l'éducation dans votre région ?</p>
                    <p>2. Quelles améliorations suggérez-vous pour le système éducatif ?</p>
                `;
                break;

            case 3:
                sectorTitle.innerText = "Questions sur la Sécurité";
                questionsList.innerHTML = `
                    <p>1. Comment évaluez-vous la sécurité dans votre quartier ?</p>
                    <p>2. Quelles mesures de sécurité devraient être renforcées dans votre région ?</p>
                `;
                break;

            case 4: // Autres
                sectorTitle.innerText = "Questions sur les Autres Secteurs";
                otherSectorSelectContainer.style.display = 'block'; // Afficher le select
                break;

            default:
                sectorTitle.innerText = '';
                questionsList.innerHTML = '';
                break;
        }
    }

    // Ajouter un événement pour afficher les questions spécifiques après la sélection dans "Autres"
    document.getElementById('otherSectorSelect').addEventListener('change', function() {
        const selectedValue = this.value;
        const questionsList = document.getElementById('questionsList');
        
        if (selectedValue === '') {
            questionsList.innerHTML = ''; // Pas de questions si rien n'est sélectionné
            return;
        }

        // Affichage des questions basées sur le secteur choisi dans le select
        let questionsHtml = '';
        if (selectedValue === '1') {
            questionsHtml = `
                <p>1. Quels autres secteurs vous préoccupent le plus ?</p>
                <p>2. Quelles suggestions avez-vous pour améliorer la qualité de vie dans votre communauté ?</p>
                <p>3. Quelles initiatives aimeriez-vous voir mises en place dans votre région ?</p>
            `;
        } else if (selectedValue === '2') {
            questionsHtml = `
                <p>1. Quels sont les secteurs d'activités non couverts qui vous semblent importants ?</p>
                <p>2. Comment pensez-vous que l'on pourrait améliorer la gestion des ressources locales ?</p>
            `;
        } else if (selectedValue === '3') {
            questionsHtml = `
                <p>1. Quels autres services de proximité souhaiteriez-vous voir dans votre région ?</p>
                <p>2. Quels types d'initiatives communautaires seraient bénéfiques pour vous ?</p>
            `;
        }

        // Insérer les questions dans le DOM
        questionsList.innerHTML = questionsHtml;
    });
 
    document.addEventListener("DOMContentLoaded", function() {
        // Fonction pour changer la couleur des étoiles
        function setStarColor(stars, value) {
            stars.forEach(star => {
                if (parseInt(star.getAttribute('data-value')) <= value) {
                    star.style.color = '#FFD700'; // Or pour les étoiles sélectionnées
                } else {
                    star.style.color = '#ccc'; // Gris pour les étoiles non sélectionnées
                }
            });
        }

        // Fonction pour gérer le changement de message en fonction de la satisfaction
        function updateSatisfactionMessage(value) {
            const satisfactionMessage = document.getElementById('satisfactionMessage');
            if (value === '1') {
                satisfactionMessage.textContent = '😞'; // Très Insatisfait
                satisfactionMessage.className = 'satisfaction-poor';
            } else if (value === '2') {
                satisfactionMessage.textContent = '😐'; // Neutre
                satisfactionMessage.className = 'satisfaction-medium';
            } else if (value === '3') {
                satisfactionMessage.textContent = '🙂'; // Satisfait
                satisfactionMessage.className = 'satisfaction-good';
            } else if (value === '4') {
                satisfactionMessage.textContent = '😀'; // Très Satisfait
                satisfactionMessage.className = 'satisfaction-excellent';
            } else if (value === '5') {
        ...
        // Sélectionner tous les éléments star-rating
        ...
        starRatings.forEach(starRating => {
            ...
            stars.forEach(star => {
                star.addEventListener('mouseover', () => {
                    ...
                    setStarColor(stars, value); // Changer la couleur des étoiles au survol
                });

                star.addEventListener('click', () => {
                    const value = star.getAttribute('data-value');
                    setStarColor(stars, value); // Changer la couleur des étoiles quand cliqué
                    serviceQualityInput.value = value; // Sauvegarder la note dans le champ caché
                    updateSatisfactionMessage(value); // Mettre à jour le message de satisfaction
                });

                star.addEventListener('mouseout', () => {
                    const selectedValue = serviceQualityInput.value;
                    setStarColor(stars, selectedValue); // Revenir à l'état sélectionné quand la souris quitte
                });
            });
        });
    });
...
<style>
    ...
    /* Satisfaction levels colors */
    .satisfaction-poor { color: red; }
    .satisfaction-medium { color: orange; }
    .satisfaction-good { color: green; }
    .satisfaction-excellent { color: darkgreen; }
</style>
